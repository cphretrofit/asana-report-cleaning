...
<style>
    ...
    * { font-family: 'Inter', sans-serif; }
    ...
    .info-text { font-size: 0.85rem; color: #6b7280; line-height: 1.6; }
</style>
...
# Known custom field column names to detect
KNOWN_CUSTOM_FIELDS = [
...
def extract_postcode(value):
    """Extract and normalise the postcode from an address string."""
    if pd.isna(value):
        return ""
    match = UK_POSTCODE.search(str(value))
    if not match:
        return ""
    # Normalise: ensure single space between outward and inward codes
...
def detect_custom_fields(columns):
    """Return which known custom field columns are present in the CSV."""
...
st.markdown(
    '<div class="subtitle">Upload an Asana export — '
    'subtasks and duplicates are removed automatically</div>',
    unsafe_allow_html=True,
)
...
if uploaded_file:
    ...
    # Detect which known custom fields are present
    ...
    # ── Show detected custom fields ──────────────────────────────────────
    if found_custom_fields:
        cf_list = "<br>".join(f"• {cf}" for cf in found_custom_fields)
        st.markdown(
            f'<div class="custom-box">🏷️ Custom fields detected '
            f'(will be included in output):<br>{cf_list}</div>',
            unsafe_allow_html=True,
        )
    ...
    if not parent_col:
        ...
        extract_uprn = st.checkbox(
            ...
            help="Extracts the UPRN number from parent task Notes, adds it "
                 "as a column, and removes the Notes column",
        )

        if st.button("Clean CSV"):

            # Step 1: Keep only rows where Parent task contains a UK postcode
            ...
            # Step 3: Extract postcode into its own column
            ...
            # Step 5: Pull custom field values (using original df index is fine
            #         since filtered_df retains original row indices)
            for cf in found_custom_fields:
                if cf in df.columns:
                    filtered_df[cf] = df.loc[filtered_df.index, cf].values

            # Step 6: Rename parent_col → "Address" in the output
            filtered_df = filtered_df.rename(columns={parent_col: 'Address'})

            # Step 7: Build final column order
            #   Task ID | Address | Postcode | UPRN Number | custom fields…
            ...
            # Guard: only keep columns that actually exist
            ...
            if found_custom_fields:
                summary += f" — {len(found_custom_fields)} custom field(s) included"
